...
-- 2022-02-16
ALTER TABLE `admin_roles` ADD `r_balance` ENUM('y','n') NOT NULL DEFAULT 'n' AFTER `r_export_bonus`;
ALTER TABLE `admin_roles` ADD `r_tax` ENUM('y','n') NOT NULL DEFAULT 'n' AFTER `r_balance`;
ALTER TABLE `admin_roles` ADD `r_export_tax` ENUM('y','n') NOT NULL DEFAULT 'n' AFTER `r_tax`;
