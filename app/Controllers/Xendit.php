    ...
    public function disbursement()
    {
        ...
        if (!$header) {
            ...
            try {
                ...
                // get old user_payout
                $user_payout_old = $UserPayout->getUserPayoutAndDetail([
                    'upd.id'    => $body->id,
                ], 'upd.status');
                if (!$user_payout_old) {
                    ...
                    if ($user_payout_old->status == 'COMPLETED') {
                        $response->message = "$body->id is already COMPLETED";
                    } else {
                        ...
                        $user_payout = $UserPayout->getUserPayoutAndDetail([
                            'upd.id'    => $body->id,
                        ], 'ups.type,user_balance_id,check_id,ups.user_id');
                        if (!$user_payout) {
                            $response->message = "Invalid upd.id $body->id";
                        } else {
